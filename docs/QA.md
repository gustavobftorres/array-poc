...
| `PW_CHROMIUM=… node scripts/hostile-attrname-ciclo7.mjs` | 19 nomes de atributo hostis pelo `prompt` do Playground (case, espaços, homoglifos, `data-on*`, `formaction`, `href`, `xlink:href`, `background`) e cola cada snippet num HTML em branco instrumentado com `window.__pwned` | 5 nomes passam (inertes, Y-004); 0 execuções |
| `PW_CHROMIUM=… node scripts/step-state-ciclo7.mjs` | os 6 estados de passo do Guia em 6 cenários com `localStorage` limpo; monta um componente **de verdade** stubando o CDN da Array por `page.route` (o botão é **Montar componente**, não há auto-mount) | limpa 0/6 · semeada 2/6 · enrollment 1/6 · KBA 2/6 · usertoken 3/6 · componente 1/6 |
...
| `PW_CHROMIUM=… node scripts/walkthrough-ciclo7.mjs` | 9 telas (8 rotas + rota inexistente) em desktop e mobile 390px, console, ≥400, tema (`☾ Escuro`/`☀ Claro`), F5 no meio do fluxo e a coerência da contagem de usuários; grava `docs/screenshots/qa7-*` | 1 achado (Y-006) |
...
- **`limit`/`offset` em `GET /api/array/users`** (desde o ciclo 7): default `limit=50`, teto 200;
  valor não-inteiro cai no default, mas **string vazia cai no mínimo** (`?limit=` devolve 1 linha —
  Y-007). A UI do Dashboard ainda não passa nem lê esses parâmetros, então acima de 50 usuários a
  contagem na tela fica menor que o `total` (Y-006).
